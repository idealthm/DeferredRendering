...
SamplerInterfaceBlock::Builder&
SamplerInterfaceBlock::Builder::stageFlags(RHI::ShaderStageFlags stageFlags) {
    mStageFlags = stageFlags;
    return *this;
}

SamplerInterfaceBlock::Builder& SamplerInterfaceBlock::Builder::add(
        std::string_view samplerName, Binding binding, Type type, Format format, bool multisample) noexcept {
    mEntries.push_back({
            { samplerName.data(), samplerName.size() }, // name
            { }, // uniform name
            binding, type, format, multisample });
    return *this;
}
...
SamplerInterfaceBlock::Builder& SamplerInterfaceBlock::Builder::add(
    ...
    for (auto& e : list) {
        add(e.name, e.binding, e.type, e.format, e.multisample);
    }
    return *this;
}
