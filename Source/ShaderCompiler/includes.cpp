...
bool resolveIncludes(IncludeResult& root, IncludeCallback callback,
    ...
    if (depth > 30) {
        ...
        std::cout << "Include depth > 30. Include cycle?" << std::endl;
        return false;
    }
    ...
    auto insertLineDirective = [&options](std::ostream& stream, size_t line, const char* filename) {
    ...
    for (auto it = includes.rbegin(); it < includes.rend() && options.insertLineDirectives; ++it) {
        ...
        std::stringstream closingDirective;
