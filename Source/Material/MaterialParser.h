...
struct ChunkUib
{
	...
	static bool Serialize(FArchive& ar, Container& obj)
	{
		...
		{
			ar << field.name << field.offset << field.stride << field.type
			   << field.isArray << field.size << field.structName << field.sizeName;
		}
		return true;
	}
};
...
struct ChunkSib
{
	...
	static bool Serialize(FArchive& ar, Container& obj)
	{
		...
		for (auto& sampler : obj.mSamplersInfoList)
		{
			ar << sampler.name << sampler.sampler << sampler.format << sampler.binding;
		}
		return true;
	}
};
...
struct ChunkDescriptorSetLayout
{
	...
	using Container = Program::DescriptorSetInfo;
	static bool Serialize(FArchive& ar, Container& obj)
	{
		for (auto& bindingSet : obj)
		{
			uint32_t count = ar.IsLoading() ? 0 : static_cast<uint32_t>(bindingSet.size());
			ar << count;
			if (ar.IsLoading())
				bindingSet.resize(count);
			for (auto& desc : bindingSet)
			{
				ar << desc.name << desc.type << desc.binding;
			}
		}
		return true;
	}
