...
template<typename Enum, typename std::enable_if_t<
        std::is_enum_v<Enum> && EnableIntegerOperators<Enum>::value, int> = 0>
...
template<typename Enum, typename std::enable_if_t<
        std::is_enum_v<Enum> && EnableBitMaskOperators<Enum>::value, int> = 0>
inline constexpr bool operator!(Enum rhs) noexcept {
    using underlying = std::underlying_type_t<Enum>;
    return underlying(rhs) == 0;
}

template<typename Enum, std::enable_if_t<
			std::is_enum_v<Enum> && EnableBitMaskOperators<Enum>::value, int> = 0>
