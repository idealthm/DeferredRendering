...
static std::string locstring(const Location &loc)
{
    std::ostringstream ostr;
...
static bool isAlternativeBinaryOp(const Token *tok, const std::string &alt)
{
    return (tok->name &&
            tok->str() == alt &&
            tok->previous &&
...
static bool isAlternativeUnaryOp(const Token *tok, const std::string &alt)
{
...
void Location::adjust(const std::string &str)
{
...
namespace {
    ...
    public:
        /**
         * @throws Output thrown if file is not found
         */
        // cppcheck-suppress uninitDerivedMemberVar - we call Stream::init() to initialize the private members
        explicit FileStream(const std::string &filename, std::vector<std::string> &files)
        {
            ...
            if (!file) {
                ...
                throw Output(Output::FILE_NOT_FOUND, {}, "File is missing: " + filename);
            }
            init();
        }
...
TokenList::TokenList(std::vector<std::string> &filenames) : frontToken(nullptr), backToken(nullptr), files(filenames) {}

TokenList::TokenList(std::istream &istr, std::vector<std::string> &filenames, const std::string &filename, OutputList *outputList)
...
TokenList::TokenList(const unsigned char* data, std::size_t size, std::vector<std::string> &filenames, const std::string &filename, OutputList *outputList, int /*unused*/)
    : frontToken(nullptr), backToken(nullptr), files(filenames)
{
...
void TokenList::dump(bool linenrs) const
{
    std::stringstream ss;
    stringify(ss, linenrs);
    std::cout << ss.str() << std::endl;
}

void TokenList::stringify(std::stringstream& ss, bool linenrs) const
{
    Location loc;
    loc.line = 1;
    bool filechg = true;
...
static void portabilityBackslash(OutputList *outputList, const Location &location)
{
    if (!outputList)
        return;
    Output err{
        Output::PORTABILITY_BACKSLASH,
        location,
...
void TokenList::readfile(Stream &stream, const std::string &filename, OutputList *outputList)
{
    std::stack<Location> loc;
...
static bool isAlternativeAndBitandBitor(const Token* tok)
{
...
std::string TokenList::readUntil(Stream &stream, const Location &location, const char start, const char end, OutputList *outputList)
{
    std::string ret;
    ret += start;
...
const Token* TokenList::isLastLinePreprocessor(int maxsize) const
{
...
const std::string& TokenList::file(const Location& loc) const
{
 ...
 */
static void simplifySizeof(TokenList &expr, const std::map<std::string, std::size_t> &sizeOfType)
{
    for (Token *tok = expr.front(); tok; tok = tok->next) {
        ...
        std::string type;
        for (const Token *typeToken = tok1; typeToken != tok2; typeToken = typeToken->next) {
...
static std::string openHeader(std::ifstream &f, const DUI &dui, const std::string &sourcefile, const std::string &header, bool systemheader);
...
static void simplifyHasInclude(TokenList &expr, const DUI &dui)
{
    for (Token *tok = expr.front(); tok; tok = tok->next) {
        ...
        if (systemheader) {
            ...
            for (const Token *headerToken = tok1->next; headerToken != tok3; headerToken = headerToken->next)
 ...
 */
static long long evaluate(TokenList &expr, const DUI &dui, const std::map<std::string, std::size_t> &sizeOfType)
{
...
std::pair<FileData *, bool> FileDataCache::tryload(FileDataCache::name_map_type::iterator &name_it, const DUI &dui, std::vector<std::string> &filenames, OutputList *outputList)
{
...
FileDataCache load(const TokenList &rawtokens, std::vector<std::string> &filenames, const DUI &dui, OutputList *outputList, FileDataCache cache)
{
#ifdef SIMPLECPP_WINDOWS
...
static bool preprocessToken(TokenList &output, const Token *&tok1, MacroMap &macros, std::vector<std::string> &files, OutputList *outputList)
{
    const Token * const tok = tok1;
    const MacroMap::const_iterator it = tok->name ? macros.find(tok->str()) : macros.end();
    if (it != macros.end()) {
        TokenList value(files);
        try {
            tok1 = it->second.expand(value, tok, macros, files);
        } catch (const Macro::Error &err) {
            if (outputList) {
                Output out{
                    Output::SYNTAX_ERROR,
                    err.location,
...
void preprocess(TokenList &output, const TokenList &rawtokens, std::vector<std::string> &files, FileDataCache &cache, const DUI &dui, OutputList *outputList, std::list<MacroUsage> *macroUsage, std::list<IfCond> *ifCond)
{
#ifdef SIMPLECPP_WINDOWS
    if (dui.clearIncludeCache)
        nonExistingFilesCache.clear();
        ...
        if (rawtok->op == '#' && !sameline(rawtok->previousSkipComments(), rawtok)) {
            ...
            if (rawtok->str() == DEFINE) {
                ...
                if (filedata == nullptr) {
                    ...
                    if (outputList) {
                        Output out{
                            Output::INCLUDE_NESTED_TOO_DEEPLY,
                            rawtok->location,
